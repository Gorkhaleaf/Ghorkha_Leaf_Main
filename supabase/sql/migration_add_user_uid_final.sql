-- Migration: add user_uid FK to orders (Postgres)
-- Paste this whole script into the Supabase SQL editor and run as a single statement.
...
-- 4) Add FK constraint if it doesn't already exist (safe check)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 1) Inspect distinct non-UUID user_id values:
...
--    COMMIT;
--
-- 3) If user_id contains non-UUID external ids, DO NOT run the text-copy. Instead:
--    - Create a mapping between external ids and auth.users.id, then update user_uid accordingly, OR
--    - Leave user_uid NULL and use the claim endpoint (/api/debug/claim-orders) / manual reconciliation.