-- Migration: add user_uid FK to orders
-- Run this migration in the Supabase SQL editor or with your migration runner.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_orders_user_uid'
  ) THEN
...
-- Find recent unclaimed orders that have a payment id:
-- SELECT id, user_id, user_uid, razorpay_order_id, razorpay_payment_id, status, created_at
-- FROM public.orders
...
-- SAFE FALLBACK (only if the automatic cast UPDATE above fails due to non-UUID user_id values):
...
--    COMMIT;
--
-- 3) If user_id contains external/non-UUID ids, do NOT run the text-copy. Instead:
--    - Create a mapping from external ids -> auth.users.id and update user_uid accordingly, OR
--    - Leave user_uid NULL and use the claim endpoint (/api/debug/claim-orders) or manual reconciliation.